      ...
      {insight && (
        <Panel style={{marginBottom:16}}>
          ...
          <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:14}}>
            ...
            {insight.best_hook && <Pill text={`最强Hook：${insight.best_hook}`} color={C.purple}/>}
          </div>
          {insight.recommendations && (
            <div>
              {(Array.isArray(insight.recommendations)?insight.recommendations:[insight.recommendations]).map((r,i)=>(
          ...
          {insight.tomorrow_idea && (
            <div style={{marginTop:14,background:C.blue+"11",border:`1px solid ${C.blue}33`,borderRadius:10,padding:12}}>
        ...
        <Panel>
          ...
          {[...posts].sort((a,b)=>b.engagement_rate-a.engagement_rate).slice(0,5).map((post,i)=>(
            <div key={post.id} style={{display:"flex",gap:12,padding:"10px 0",borderBottom:`1px solid ${C.border}`,alignItems:"center"}}>
              ...
              {post.thumbnail_url && <img src={post.thumbnail_url} style={{width:40,height:40,borderRadius:8,objectFit:"cover",flexShrink:0}} onError={e=>e.target.style.display="none"}/>}
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{display:"flex",gap:6,marginTop:4,flexWrap:"wrap"}}>
                  ...
                  <Pill text={`❤️ ${post.likes}`} color={C.pink}/>
                  <Pill text={`🔖 ${post.saves}`} color={C.purple}/>
                  <Pill text={`👁 ${post.reach}`} color={C.blue}/>
                </div>
              </div>
              <div style={{color:C.green,fontWeight:800,fontSize:16,flexShrink:0}}>{post.engagement_rate}%</div>
            </div>
          ))}
        </Panel>
      )}
    ...
    <div>
      <Panel style={{marginBottom:16}}>
        ...
        <div style={{color:C.muted,fontSize:13,marginBottom:16,lineHeight:1.7}}>
          基于你最近 {posts.length} 条真实帖子数据，Claude 会分析：<br/>
          最爆内容类型、最佳发布时间、最有效的 Hook、明天该发什么
        </div>
        ...
        <Panel>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
            {[
              ...
              {l:"最强 Hook 方式",v:insight.best_hook,c:C.purple},
            ...
            <div>
              ...
              {(Array.isArray(insight.recommendations)?insight.recommendations:[]).map((r,i)=>(
                <div key={i} style={{display:"flex",gap:10,padding:"10px 0",borderBottom:`1px solid ${C.border}`}}>
                  <div style={{color:C.green,fontWeight:700,flexShrink:0,fontSize:16}}>✓</div>
                  <div style={{color:C.text,fontSize:13,lineHeight:1.6}}>{r}</div>
                </div>
              ))}
            </div>
...
function CaptionView() {
  ...
  const [goal,setGoal]=useState("涨粉"); const [account,setAccount]=useState("jes_watches_official");
    ...
    <div>
      <Panel style={{marginBottom:16}}>
        ...
        <div style={{display:"flex",gap:10}}>
          <input value={topic} onChange={e=>setTopic(e.target.value)} placeholder="内容主题（如：第一年创业亏了RM50K）"
...
export default function App() {
  ...
  async function loadData() {
    ...
    // Just load from supabase on init
    try {
      const {createClient} = await import("@supabase/supabase-js");
      const sb = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL||"",process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY||"");
      const {data:p} = await sb.from("posts").select("*").order("published_at",{ascending:false});
      if(p) setPosts(p);
      const {data:i} = await sb.from("ai_insights").select("*").order("created_at",{ascending:false}).limit(1).single();
      if(i) setInsight(i);
    } catch{}
  }
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,color:C.text,fontFamily:"'Inter',-apple-system,sans-serif",display:"flex",flexDirection:"column"}}>
      ...
      {/* Toast */}
      {toast && (
        <div style={{position:"fixed",top:70,left:"50%",transform:"translateX(-50%)",background:toast.color,color:"#fff",borderRadius:10,padding:"10px 20px",fontSize:13,fontWeight:600,zIndex:100,boxShadow:"0 4px 20px #0006"}}>
          {toast.msg}
        </div>
      )}
