...
想要结果：${strategy.goal}
内容风格：${strategy.style}
语言：${strategy.language}` : "马来西亚华人电商账号";
    ...
    const res = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: "user",
          content: `你是专业Instagram内容分析师，专门服务马来西亚华人账号。

${strategyContext}

分析这${posts.length}条帖子数据：
  ...
  "video_analysis": [
    ...
    try { analysis = JSON.parse(match ? match[0] : text); }
      ...
      analysis = {
        ...
        best_time: "周五晚上9点",
        best_hook: "用真实故事和数字开场",
        top_topics: ["电商经验", "创业故事", "马来西亚"],
        tomorrow_idea: "分享一个你亏过钱但学到东西的真实故事",
        recommendations: ["每周至少3条Reel", "用真实故事建立信任", "结尾加强CTA"],
        video_analysis: []
      };
    }
